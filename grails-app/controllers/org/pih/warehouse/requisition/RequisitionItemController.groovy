        ...
        if (requisitionItems) {
            ...
            requisitionItems.each { requisitionItem ->
                csv.printRecord(
                        ...
                        requisitionItem?.requisition?.name,
                        formatDate(date: requisitionItem?.requisition?.dateRequested, format: "dd/MMM/yyyy"),
                        requisitionItem?.requisition?.requestedBy?.name,
                        requisitionItem?.product?.productCode,
                        requisitionItem?.product?.name,
                        requisitionItem?.product?.genericProduct?.name,
                        requisitionItem?.cancelReasonCode,
                        requisitionItem?.quantityCanceled,
                        requisitionItem?.quantity,
                        "EA/1"  // FIXME should this pull from the item itself?
                )
            }
